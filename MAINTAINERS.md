...
Step 4: if the next version is a major version, determine if the breaking changes are caused by the action of by upgrading to a new core.
...
Step 6: Tag the current commit also with the shortened version number (e.g. `v3`), using `git tag --force v3 <commitish>` to overwrite the tag locally, then force-push the changed tag. This means that consumers using the short tag will get the latest release as well.
